...
export default function Shop({initialProducts,isAdmin=false}:{initialProducts:Product[];isAdmin?:boolean}){
  ...
  const best=[...initialProducts].sort((a,b)=>(b.salesCount??0)-(a.salesCount??0)).slice(0,8),hero=initialProducts.slice(0,2);
  ...
  return <>
    ...
    <main className="shell">
      ...
      <section className="editorial-banner"><div><span>NEW SEASON</span><h2>간절기의 온도,<br/>가볍고 섬세하게</h2><p>출근부터 주말 약속까지 이어지는 데일리 셋업</p></div>{hero[1]&&<img src={hero[1].image} alt={hero[1].name}/>}</section>
