...
export default function StoreView({
  ...
  useEffect(
    () => localStorage.setItem('somi_cart', JSON.stringify(cart)),
    [cart],
  );
  const list =
    mode === 'wishlist'
  ...
  return (
    <main className="sub-page">
      ...
      {mode !== 'cart' && !list.length && (
        <p className="empty">준비된 상품이 없어요.</p>
      )}
      {mode === 'cart' && (
              ...
              {cart.map((x, i) => (
                <div className="cart-row" key={i}>
                  ...
                  <div>
                    <b>{x.product.name}</b>
                    <small>{x.color}</small>
                    <strong>{money(x.product.price * x.qty)}원</strong>
                  </div>
