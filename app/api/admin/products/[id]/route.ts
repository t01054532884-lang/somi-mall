  ...
  try {
    ...
    await getD1()
      ...
      .bind(
        ...
        product.originalPrice, product.imageUrl, product.description, JSON.stringify(product.detailImages),
        product.material, product.origin, product.manufacturer, JSON.stringify(product.sizeChart),
        product.sellerName, product.sellerRepresentative, product.sellerAddress,
        product.sellerBusinessNumber, product.sellerMailOrderNumber, product.sellerEmail, product.sellerPhone,
        JSON.stringify(product.colors),
        product.badge, product.todayDispatch ? 1 : 0, product.active ? 1 : 0, product.saleStatus, product.styleTag,
        product.sortOrder, product.stock, id,
      )
      .run();
