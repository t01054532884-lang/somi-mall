...
type AdminPageProps = { searchParams?: Promise<{ status?: string; orderTab?: string }> };
const messages: Record<string, string> = {
  ...
  order_updated:'주문 상태와 배송 정보를 저장했습니다.',order_error:'주문 정보를 처리하지 못했습니다.',order_stock_error:'재고가 부족해 주문 상태를 되돌리지 못했습니다.',
};

export default async function Admin({ searchParams }: AdminPageProps) {
  const signedInUser = await getChatGPTUser();
  const admin = await getAdminUser();
  const query=await searchParams,status=query?.status,orderTab=query?.orderTab??'진행 주문';
  ...
  try { [products,reviews,inquiries,orders] = await Promise.all([listAdminProducts(),listAdminReviews(),listAdminInquiries(),listAdminOrders()]); } catch (error) { databaseReady = false; console.error('Unable to load admin data', error); }
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-orders" id="orders"><div className="section-head"><div><span className="eyebrow">ORDERS</span><h1>주문·배송 관리</h1></div><a className="solid" href="/api/admin/orders/export">전체 주문 CSV 다운로드</a></div><p className="note">결제 API 연결 전 주문 접수 상태로 저장됩니다. 기본 택배사는 로젠택배이며 운송장 번호를 직접 입력할 수 있습니다.</p><nav className="admin-tabs">{['진행 주문','취소','반품'].map(tab=><a key={tab} className={orderTab===tab?'selected':''} href={`/admin?orderTab=${encodeURIComponent(tab)}#orders`}>{tab} <b>{orders.filter(order=>tab==='진행 주문'?!['취소','반품'].includes(order.status):order.status===tab).length}</b></a>)}</nav>{orders.filter(order=>orderTab==='진행 주문'?!['취소','반품'].includes(order.status):order.status===orderTab).map(order=><article className="admin-card order-card" key={order.id}><header><div><b>{order.orderNumber}</b><small>{adminDate(order.createdAt)} · {order.memberName} · {order.phone}</small></div><em>{order.status}</em></header><p>{order.items.map(item=>`${item.productName} (${item.optionName}) × ${item.quantity}`).join(', ')}</p><p><b>{order.total.toLocaleString('ko-KR')}원</b> · {order.recipient} · {order.address}</p>{order.memo?<small>배송메모: {order.memo}</small>:null}<form action={`/api/admin/orders/${order.id}`} method="post" className="order-form"><label className="field">주문 상태<select name="status" defaultValue={order.status}>{['신규 주문','배송 준비','배송중','배송완료','취소','반품'].map(x=><option key={x}>{x}</option>)}</select></label><label className="field">로젠택배 운송장<input name="trackingNumber" defaultValue={order.trackingNumber} placeholder="운송장 번호"/></label><button className="solid" type="submit">주문 저장</button></form></article>)}{!orders.length?<p className="note">접수된 주문이 없습니다.</p>:null}</section>
...
function ProductFields({ product }: { product?: Awaited<ReturnType<typeof listAdminProducts>>[number] }) {
  return <div className="admin-grid">
    ...
    <label className="field">재고 수량<input name="stock" type="number" min="0" required defaultValue={product?.stock ?? 10} /></label>
