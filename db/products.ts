...
export type ProductReview={id:string;memberName:string;rating:number;content:string;adminReply:string;createdAt:string};
export async function listProductReviews(productId:string):Promise<ProductReview[]>{const result=await getD1().prepare('SELECT id, member_name, rating, content, admin_reply, created_at FROM reviews WHERE product_id = ? ORDER BY created_at DESC').bind(productId).all<{id:string;member_name:string;rating:number;content:string;admin_reply:string;created_at:string}>();return result.results.map(r=>({id:r.id,memberName:r.member_name,rating:r.rating,content:r.content,adminReply:r.admin_reply,createdAt:r.created_at}))}
...
export type AdminReview=ProductReview&{productId:string;productName:string};
export async function listAdminReviews():Promise<AdminReview[]>{const result=await getD1().prepare('SELECT reviews.id, reviews.product_id, products.name AS product_name, reviews.member_name, reviews.rating, reviews.content, reviews.admin_reply, reviews.created_at FROM reviews JOIN products ON products.id = reviews.product_id ORDER BY reviews.created_at DESC').all<{id:string;product_id:string;product_name:string;member_name:string;rating:number;content:string;admin_reply:string;created_at:string}>();return result.results.map(r=>({id:r.id,productId:r.product_id,productName:r.product_name,memberName:r.member_name,rating:r.rating,content:r.content,adminReply:r.admin_reply,createdAt:r.created_at}))}
export type AdminInquiry=ProductInquiry&{productId:string;productName:string};
export async function listAdminInquiries():Promise<AdminInquiry[]>{const result=await getD1().prepare('SELECT product_inquiries.id, product_inquiries.product_id, products.name AS product_name, product_inquiries.member_name, product_inquiries.content, product_inquiries.answer, product_inquiries.created_at FROM product_inquiries JOIN products ON products.id = product_inquiries.product_id ORDER BY product_inquiries.created_at DESC').all<{id:string;product_id:string;product_name:string;member_name:string;content:string;answer:string;created_at:string}>();return result.results.map(r=>({id:r.id,productId:r.product_id,productName:r.product_name,memberName:r.member_name,content:r.content,answer:r.answer,createdAt:r.created_at}))}
